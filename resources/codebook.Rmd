...
The titanic data can be loaded by

```{r, message=F, warning=F}
titanic <- read_csv("https://sta214-f25.github.io/homework/Titanic.csv")
```
...
They have collected the following variables for each loan in the data:
...
This dataset contains information on the publication records of 915 biochemistry PhD students, with variables
...
library(palmerpenguins)
library(tidyverse)
titanic <- read_csv("https://sta214-f22.github.io/labs/Titanic.csv")
```
...
Before fitting a logistic regression model:
...
Before fitting a logistic regression model, we can use empirical logit plots to visualize the relationship between the binary response and individual quantitative explanatory variables. 

Here is a function I have written to allow you to create an empirical logit plot:
...
And here is an example of using the function with the titanic data:
...
* The first argument is the dataset (in this case `titanic`)
* The second argument specifies the response and explanatory variables (provided as a formula, just like we would do in a call to `glm`)
* `num_bins` specifies the number of bins to use (i.e., the number of resulting points in the empirical logit plot)

From the plot, it looks like we may need a transformation. Let's try a square root:

```{r}
logodds_plot(titanic, Survived ~ sqrt(Fare), 
             num_bins = 20)
```
...
### Checking for multicollinearity -- scatterplots and correlation

*Multicollinearity* refers to the presence of multiple explanatory variables which are highly correlated. When we have correlated explanatory variables in our model, it can cause issues for our estimates of model coefficients.

One way to check for multicollinearity is by assessing scatterplots of pairs of quantitative explanatory variables, and calculating the correlation coefficient for each pair. This can be done with the `ggpairs` function in the `GGally` package. For example, with the SBA data:

```{r, message=F, warning=F}
library(tidyverse)
library(GGally)

sba <- read_csv("https://sta214-f25.github.io/class_activities/sba_small.csv")
...
From this plot, we can see that the `GrAppv`, `SBA_Appv`, and `DisbursementGross` variables are all highly correlated -- as each of these variables measures a related aspect of the loan amount, this should not surprise us!

This suggests that we may wish to choose only one of these three variables (e.g., `DisbursementGross`), or somehow combine these three variables into a single variable.

### Checking for multicollinearity -- VIFs

A *variance inflation factor* (VIF) measures how much the variability of a model's estimated coefficients increases when other explanatory variables are added to the model. The more correlation there is between explanatory variables, the harder it is to reliably estimate the model coefficients (the $\beta$s), and so the more variability there will be in the coeffficient estimates (the $\widehat{\beta}$s).

Variance inflation factors can be calculated after we fit a model, to do a second check for multicollinearity. Variance inflation factors can be calculated with the `vif` function from the `car` package.

For example, suppose we ignored the scatterplots and correlations from the previous section, and fit a model with the SBA data:

```{r, message=F, warning=F}
library(car)

m_sba <- glm(Default ~ GrAppv + SBA_Appv + 
...
The VIFs for the three correlated variables are *very* high! All VIF values will be $> 1$, and a general rule of thumb is to be concerned by VIFs above 5.

Now if we fit a more reasonable model, choosing only one of the three correlated variables, we get a lower VIF for `DisbursementGross`:

```{r}
m_sba <- glm(Default ~ DisbursementGross + Term,
             data = sba, family = binomial)

vif(m_sba)
```

These two VIFs are about as low as possible (remember the lower bound is 1).

### Influential points and Cook's distance

*Outliers* are points which do not follow the general trend of the data. For example, suppose that we have data from the following logistic regression model:

$$Y_i \sim Bernoulli(\pi_i)$$

$$\log \left( \dfrac{\pi_i}{1 - \pi_i} \right) = -1 + 2 X_i$$

If $X_i = -2$, then $\pi_i = 0.0067$, meaning that observing $Y_i = 1$ for this value of $X_i$ is quite unusual. The point $(X_i, Y_i) = (-2, 1)$ would therefore be an *outlier* in the data.

Outliers are concerning because they have the potential to *influence* the fit of our model. A point is *influential* if it substantially changes the estimated coefficients of the model. So how do we determine if a point is influential?
...
As a general rule of thumb, we are concerned about values of Cook's distance bigger than 0.5 or 1. Here is a plot of Cook's distance for the SBA data:

```{r}
plot(m_sba$fitted.values, cooks.distance(m_sba),
     xlab = "Predicted probabilities", 
     ylab = "Cook's distance")
```

All Cook's distance values are very small, so we are not concerned about influential points; the largest is only around 0.06:

```{r}
max(cooks.distance(m_sba))
```

#### Handling influential points

If we do identify an outlier, how should we handle it?

* If it is clearly an error in the data (e.g., a negative value for a variable that can't possibly be negative, like body mass), then it can be removed
* Otherwise, check Cook's distance
    * For a large Cook's distance: report the results with and without the influential point(s). That is, report the estimated coefficients, predictions, hypothesis tests, conclusions, etc.
...
Another method for assessing goodness-of-fit is a **rootogram**, which compares the frequencies of the observed counts to the expected counts from the fitted models:

```{r}
# note: to install the topmodels package, run the following
...
Bars below 0 mean we observed *more* occurrences that predicted by the model, while bars above 0 mean we observerd *fewer* occurrences than predicted by the model. In this plot, you can see that 0 articles occurs more than expected, as does 4, 5, 6, and 7 articles. On the other hand, there were fewer instances of 1, 2, and 3 publications than the model expected.

This pattern means that the tails of the observed data are heavier than predicted by the model, so there is more variability in our data than the model accounts for. In other words, overdispersion!
...
Rootograms can be used to assess negative binomial models too. Above we saw that the Poisson model for the articles data did not account for all the variability in the response. Here we see that the negative binomial model does a better job:

```{r}
rootogram(m_art_nb)
```

The bars are much closer in general to the horizontal line at 0 -- in other words, the observed counts more closely match the expected counts from the model.
...
A natural question is, **how do we choose which of these models to use?** Quantile residual plots and rootograms can be helpful here. Below are four quantile residual plots and the corresponding rootograms: 
...
p11 <- rootogram(sim_m1, main = "(a) Poisson regression on Poisson data",
                 plot=F) |> autoplot() +
  labs(title = "(a) Poisson regression on Poisson data")
...
p22 <- rootogram(sim_m3, 
                 main = "(b) Poisson regression on overdispersed data",
                 plot = F) |> autoplot() +
  labs(title = "(b) Poisson regression on overdispersed data")
...
p33 <- rootogram(sim_m1, main = "(c) Poisson regression on negative binomial data",
                 plot = F) |> autoplot() +
  labs(title = "(c) Poisson regression on negative binomial data")
...
p44 <- rootogram(sim_m2, 
                 main = "(d) Negative binomial regression on negative binomial data",
                 plot = F) |> autoplot() +
  labs(title = "(d) Negative binomial regression on negative binomial data")

grid.arrange(p1, p2, p3, p4, ncol=2)
grid.arrange(p11, p22, p33, p44, ncol=2)
```

* (a): data come from a Poisson distribution, and we fit a Poisson regression model. Most quantile residuals are between -2 and 2, and the variance of the quantile residuals appears constant. The rootogram shows no signs of overdispersion, with the bars generally falling close to the horizontal line at 0. So, a Poisson regression model is appropriate.
* (b): here we have overdispersed data (variance is too large for a Poisson), but we fit a Poisson model anyway. Variance is constant, but the spread of the quantile residuals is too large (many residuals above 3), so either a quasi-Poisson or negative binomial model could be used. The overdispersion is also visible in the rootogram, with bars in the middle falling about the horizontal line at 0, and bars at the tails falling below the line.
* (c): Here we have data from a negative binomial distribution, but we fit a Poisson model anyway. There is clear non-constant variance, and the spread of the residuals is too large. Overdispersion is also visible in the rootogrma. A negative binomial model should be used.
* (d): Same data as in (c), but now we fit the (correct) negative binomial model. The plots look much better!
...
* For the Poisson quantile residual plots, look at the spread of the residuals. Also create a rootogram.
* If the rootogram shows no clear evidence of overdispersion, and the quantile residuals have constant variance and are mostly between -2 and 2 (and almost all between -3 and 3), then a Poisson distribution is reasonable!
* If the rootogram shows overdispersion and the residuals have constant variance but the spread is too large, choose either a quasi-Poisson or negative binomial
* If the rootogram shows overdispersion and the residuals have non-constant variance, choose a negative binomial
...
wdrinks <- read_csv("https://sta279-s22.github.io/slides/weekendDrinks.csv")
...
```{r, include=F}
heart_data <- read_csv("https://sta214-f22.github.io/class_activities/framingham.csv")
...
# Writing math in Quarto and R Markdown

If you want to write mathematical notation, we need to tell Markdown, “Hey, we’re going to make a math symbol!” To do that, you use dollar signs. For instance, to make $\widehat{\beta}_1$, you simply put `$\widehathat{\beta}_1$` into the white space (not a chunk) in your Quarto or R Markdown.
