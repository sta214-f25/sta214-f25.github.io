...
**Solution:** No, it appears that there is not a linear relationship between Fare and the log odds of survival.
...
logodds_plot(titanic, Survived ~ log(Fare + 1), 
             num_bins = 20)
```

Either the square root or the log transform seem reasonable here (note that the log transform requires we add 1 first, since there are fares of 0 in the data).
...
**Solution:** When the assumptions are satisfied, the quantile residual plot looks good -- a nice random scatter of the residuals around the horizontal line at 0, with no clear patterns or trends.
...
**Solution:** Suppose we now make the log-odds a quadratic function of `x`, but assume a linear function when we fit the model. Then, we can see a clear violation of the shape assumption in the quantile residual plot:
...
data.frame(x = x, residuals = qresid(m1)) %>%
  ...
  theme_bw()
```
However, if we fit the *correct* model (with `x^2` as the predictor, instead of `x`), we can see that the quantile residual plot looks good:

```{r, message=F, warning=F}
m2 <- glm(y ~ I(x^2), family = binomial)
data.frame(x = x, residuals = qresid(m2)) %>%
  ggplot(aes(x = x, y = residuals)) +
  geom_point() +
  geom_smooth() +
  theme_bw()
